...
class EqualizerUI:
    ...
    def value_determined(self, event = None):
        ...
        if self.switch_vars['LPB'].get() == True:
            ...
            analog_a *= [1 / (2 * np.pi * self.scales['LPB'].get() / sampling_rate), 1] # Denominator
        ...
        if self.switch_vars['HPB'].get() == True:
            ...
            analog_a *= [1, 2 * np.pi * self.scales['HPB'].get() / sampling_rate] # Denominator
    ...
    def plot(self, digital_b, digital_a, sampling_rate):
        ...
        worN = [start_freq * pow(2, i / 5.0) for i in xrange(60)]
    ...
    def pop_up_box(self, main_form):
        ...
        # peak filter 1
        self.add_filter('PK1', 'Peak 1\nCenter', 20, 100, column_index = 4, column_span = 2)
        self.add_filter('PK1-Width', 'Peak 1\nBand Width', 1, 400, column_index = 5, show_switch = False)
        # peak filter 2
        self.add_filter('PK2', 'Peak 2\nCenter', 100, 500, column_index = 6, column_span = 2)
        self.add_filter('PK2-Width', 'Peak 2\nBand Width', 5, 2000, column_index = 7, show_switch = False)
        # peak filter 3
        self.add_filter('PK3', 'Peak 3\nCenter', 500, 2000, column_index = 8, column_span = 2)
        self.add_filter('PK3-Width', 'Peak 3\nBand Width', 20, 8000, column_index = 9, show_switch = False)
        # peak filter 4
        self.add_filter('PK4', 'Peak 4\nCenter', 2000, 20000, column_index = 10, column_span = 2)
        self.add_filter('PK4-Width', 'Peak 4\nBand Width', 100, 20000, column_index = 11, show_switch = False)        
        
        self.canvas = FigureCanvasTkAgg(self.fig, master = self.frame)
        self.canvas.get_tk_widget().grid(row = 4, column = 0, columnspan = 12, ipady = 40)        
